...
var (
	ErrQueryUser         = errors.New("query failed")
	ErrScanUser          = errors.New("scan failed")
	ErrConcatenatingUser = errors.New("error concatenating user values from DB")
	ErrIterationRowsUser = errors.New("rows iteration error")
)
...
func (r *userRepository) GetAll(ctx context.Context) ([]*users.User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(got, ErrQueryUser, err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf(got, ErrScanUser, err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf(got, ErrConcatenatingUser, err)
		}
	...
	if err = rows.Err(); err != nil {
		return nil, fmt.Errorf(got, ErrIterationRowsUser, err)
	}
...
func (r *userRepository) ExistsById(ctx context.Context, id uuid.UUID) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf(got, ErrQueryUser, err)
	}
...
func (r *userRepository) Delete(ctx context.Context, u *users.User) (*users.User, error) {
	var (
		id                                                                        uuid.UUID
		firstName, lastName, username, email, password, gender, country, language string
		birth, lastLoginAt, createdAt, updatedAt                                  time.Time
	...
	err := r.DB.QueryRowContext(ctx, QueryDeleteUser, u.Id(), u.DeletedAt()).Scan(
		&id, &firstName, &lastName, &username, &email, &password, &gender, &birth, &country, &language, &phone, &information, &profilePic, &webSite, &visibility, &lastLoginAt, &createdAt, &updatedAt, &deletedAt,
	)
	if err != nil {
		return nil, fmt.Errorf(got, ErrQueryUser, err)
